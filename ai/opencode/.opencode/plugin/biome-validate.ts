...
const buildMessage = (
  ...
  const sections = [
    `Code validation detected failures in ${sessionName}.`,
    `Summary:\n${summary}`,
  ];
...
export const BiomeValidate: Plugin = ({ $, client, directory }) => {
  ...
  return Promise.resolve({
    event: async ({ event }) => {
      ...
      try {
        ...
        debug("failure", error);
        await $`notify-send -u normal -t ${NOTIFICATION_EXPIRE_MS} ${sessionName} "Biome validation plugin failed"`
          .quiet()
          .nothrow();
